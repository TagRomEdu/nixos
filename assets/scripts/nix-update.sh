...
updates="$(cd ~/nixos && nix flake lock --update-input nixpkgs && nix build .#nixosConfigurations.default.config.system.build.toplevel && nvd diff /run/current-system ./result | grep -e '\[U' | wc -l)"
